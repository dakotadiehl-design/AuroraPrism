...
extension MIDIActionResolverTests {
    ...
    func testNoteVelocityControlValue() {
        let v1 = MIDIActionResolver.controlValue(for: .noteOn(channel: 0, note: 36, velocity: 1, sourceID: "t"))
        let v64 = MIDIActionResolver.controlValue(for: .noteOn(channel: 0, note: 36, velocity: 64, sourceID: "t"))
        let v127 = MIDIActionResolver.controlValue(for: .noteOn(channel: 0, note: 36, velocity: 127, sourceID: "t"))
        let off = MIDIActionResolver.controlValue(for: .noteOff(channel: 0, note: 36, velocity: 0, sourceID: "t"))
    ...
    func testCCControlValue() {
        let v = MIDIActionResolver.controlValue(for: .controlChange(channel: 0, controller: 7, value: 64, sourceID: "t"))
    ...
    func testData2VelocityFilter() {
        ...
        let match = MIDIEvent.noteOn(channel: 0, note: 36, velocity: 100, sourceID: "t")
        let miss = MIDIEvent.noteOn(channel: 0, note: 36, velocity: 50, sourceID: "t")
