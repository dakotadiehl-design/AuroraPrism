...
/// LEGACY multi-pane host. Not production ContentView root (UI-02A). Use BuildWorkspaceHost.
public struct LegacyWorkspaceView: View {
    ...
    public init(
        ...
        panelBuilder: @escaping (WorkspacePanelID, WorkspacePanelContext) -> AnyView = LegacyWorkspaceView.defaultPanel
    ) {
        self._layout = layout
    ...
    private var leadingTabs: [WorkspacePanelID] {
        [.fixtureBrowser, .patch, .groups, .cueBlocks, .palettes, .song, .effects].filter { layout.isVisible($0) }
    }
